...
public class UDPCLIENT extends javax.swing.JFrame {
    ...
    private int[][] matran;
    private InetAddress svaAddress;

    public UDPCLIENT() throws IOException {
        initComponents();
        clSocket = new DatagramSocket();
        svaAddress = InetAddress.getByName("localhost");
    }
    ...
    private void bttTimMaxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bttTimMaxActionPerformed
        try {
            byte[] matranbyte= txtaMatran.getText().getBytes();
            byte[] cn = String.valueOf(1).getBytes();
            
            udpSend(matranbyte);
            udpSend(cn);
            
            txtaServer.append(udpReceive());
    ...
    private void bttTimSoNtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bttTimSoNtActionPerformed
        try {
            byte[] matranbyte= txtaMatran.getText().getBytes();
            byte[] cn = String.valueOf(2).getBytes();
    ...
    private void bttSapXepTangDanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bttSapXepTangDanActionPerformed
        try {
            byte[] matranbyte= txtaMatran.getText().getBytes();
            byte[] cn = String.valueOf(3).getBytes();
    ...
    public void udpSend(byte[] message) throws IOException{
        DatagramPacket packet = new DatagramPacket(message, message.length, svaAddress, 2212);
    ...
    public String udpReceive() throws IOException{
        ...
        clSocket.receive(packetReceive);

        return new String(packetReceive.getData(), 0, packetReceive.getData().length);
    }
    /**
